...
static int process (jack_nframes_t nframes, void *arg)
{
	...
	ctx->rectify_db = TTFLOAT(TTLINEAR2DB(tt_analyse_rectify(ctx->input_buf)));
	ctx->peak_db = TTFLOAT(TTLINEAR2DB(tt_analyse_peak(ctx->input_buf)));

	tt_siggen_process(&ctx->siggen, ctx->output_buf);
...
int main (int argc, char *argv[])
{
	...
	*/
	...
	tt_siggen_init(&ctx->siggen, &ctx->tt_ctx);
	tt_siggen_setup(&ctx->siggen, 48000/256, TTINT(1), TT_SIGGEN_SIN);
	...
	/* display signal analysis until exit*/
	while (1) {
		printf("\rPeak  %6.2fdB   Average  %6.2fdB",
				ctx->peak_db, ctx->rectify_db);
		fflush(stdout);
		sleep (2);
	}
